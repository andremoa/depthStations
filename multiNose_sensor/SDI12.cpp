...
#define PCMSK          PCMSK0
#define PCMSKPIN       PCINT1
#define GIMSKPIN       PCIE0
...
{
  if (PIN_STATUS&(1<<PIN))				// 6.2.1 - Start bit?
...
void getDepth(uint16_t* depth){
	
	char startMeasurement[4] = {'?', 'M', '!', 0x0};
	char getData[5] = {'?', 'D', '0', '!', 0x0};
	char SDI_Response[30] = {0};
	uint16_t SDI_Response_Stripped = 0;
	
		
	SDI12 sdi12(0);
	sdi12.begin();
	sdi12.sendCommand(startMeasurement);
	_delay_ms(3000);
	sdi12.sendCommand(getData);
	_delay_ms(300);
	...
	while(sdi12.available()){
	 ...
	 uint8_t numberOfPlus = 0;
	 uint8_t plusOnePosition = 0;
	 uint8_t plusTwoPosition = 0;
	...
	{
		printString("\nYep\n");
		*depth = convertFromASCIIToBinary(SDI_Response[plusOnePosition+1])*1000 + convertFromASCIIToBinary(SDI_Response[plusOnePosition+3])*100 + convertFromASCIIToBinary(SDI_Response[plusOnePosition+4])*10 + convertFromASCIIToBinary(SDI_Response[plusOnePosition+5]);
	}
	else{
		printString("\nNope\n");
		*depth = 0;
	...
	for(int i=0; i<15; i++){
		SDI_Response[i] = convertFromASCIIToBinary(SDI_Response[i]);
	}
	
	SDI_Response_Stripped = SDI_Response[7]*10E3 + SDI_Response[8]*10E2 + SDI_Response[9]*10 + SDI_Response[10];
	
	*/
	
	/*
	*depthMSB = ((SDI_Response_Stripped >> 8) & 0xff);
	*depthLSB = (SDI_Response_Stripped & 0xff);
	*/
	
	//*depth = SDI_Response_Stripped;
