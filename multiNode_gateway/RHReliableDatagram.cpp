...
// ----- NOTE: THIS FILE HAS BEEN MODIFIED, AND IS THUS NOT THE ORIGINAL ONE BY RADIO HEAD. USE WITH CAUTION. ------
...
////////////////////////////////////////////////////////////////////
bool RHReliableDatagram::recvfromAck(uint8_t* buf, uint8_t* len, uint8_t* from, uint8_t* to, uint8_t* id, uint8_t* flags, uint8_t minutes, uint8_t seconds)
{  
    uint8_t _from;
    uint8_t _to;
    uint8_t _id;
	...
	if (1)
	{
		SerialUSB.println(_from);
		SerialUSB.println(_to);
		SerialUSB.println(_id);
		SerialUSB.println(_flags);
		*id = _id;
		*flags = _flags;
	    // Its a normal message not an ACK
	    if (_to ==_thisAddress)
		...
		// Its not a broadcast, so ACK it
		// Acknowledge message with ACK set in flags and ID set to received ID
		//acknowledge(_id, _from, minutes, seconds);
	    }
	    // If we have not seen this message before, then we are interested in it
		// _id != _seenIds[_from]
	    if (1)
...
void RHReliableDatagram::acknowledge(uint8_t id, uint8_t from, uint8_t minutes, uint8_t seconds)
{
    setHeaderId(id);
    ...
    //uint8_t ack = '!';
	uint8_t ack[2] = {minutes, seconds};
	SerialUSB.print("rtcTime.getMinutes() = ");
	SerialUSB.println(ack[0]);
	SerialUSB.print("rtcTime.getSeconds() = ");
	SerialUSB.println(ack[1]);
